...
const purchaseOrdersStart = state => {
    return updateObject(state, { loading: true });
};
...
const purchaseOrdersFail = state => {
    return updateObject(state, { loading: false });
};
...
const reducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case actionTypes.FETCH_ORDERS_START: return purchaseOrdersStart(state);
        case actionTypes.FETCH_ORDERS_SUCCESS: return purchaseOrdersSuccess(state, action);
        case actionTypes.FETCH_ORDERS_FAIL: return purchaseOrdersFail(state);
